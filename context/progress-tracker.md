...
- **模块 49：应用配置仓库读取** 已完成实现与审查：新增只读 `Database::list_applications`，按 `position ASC, id ASC` 读取五列并恢复完整 `ApplicationConfig`；拒绝无效 UUID、非规范显示名称/图标键、相对/NUL/非 `.exe` 路径、非声明 SQLite 列类型以及重复或间断位置，统一返回 `CorruptApplicationStore`，且损坏读取不写入、不修复、不丢弃数据。首轮 review 发现该错误未映射到共享 `data_corrupt` code（P2），现已补齐映射与序列化回归测试；`gpt-5.6-sol medium` 最终复审结果为 `APPROVE`，无 P1/P2。18 条应用聚焦 Rust 测试、117 条前端测试和 127 条 Rust 全量测试通过；Prettier、ESLint、TypeScript、Vite build、Rust fmt、Clippy、cargo check、git diff check 与 Tauri release build 均通过；本模块无 UI 变更，不需要视觉验收；模块提交 `63a71d1` 已推送到 `origin/main`。
- **模块 50：应用配置仓库创建** 已完成实现与审查：新增即时事务 `Database::create_application`，复用完整读取验证现有存储，以当前配置数生成尾部位置，构造规范化领域对象后写入五个持久化字段并返回；路径只做领域语法校验，不访问文件系统。测试覆盖持久化重开、无效输入原子失败、损坏存储拒绝写入和双连接并发追加；`gpt-5.6-sol medium` 独立 review 结果为 `APPROVE`，无 P1/P2。22 条应用聚焦 Rust 测试、117 条前端测试和 131 条 Rust 全量测试通过；Prettier、ESLint、TypeScript、Vite build、Rust fmt、Clippy、cargo check、git diff check 与 Tauri release build 均通过；本模块无 UI 变更，不需要视觉验收；模块提交 `831a1aa` 已推送到 `origin/main`。
- **模块 51：应用配置仓库删除** 已完成实现与审查：新增即时事务 `Database::delete_application`，写前验证完整存储，删除目标、压缩后续位置并返回权威有序快照；新增 `ApplicationNotFound` 存储错误和共享 `application_not_found` Rust/TypeScript IPC 错误契约。测试覆盖中部/首部/尾部删除及每步重开、未知 ID 原子失败和损坏存储拒绝删除。首轮 review 发现前端运行时错误白名单缺失新 code（P2），现已补齐联合类型、白名单和规范化回归测试；`gpt-5.6-sol medium` 最终复审结果为 `APPROVE`，无 P1/P2。26 条应用聚焦 Rust 测试、44 条 IPC 客户端聚焦测试、118 条前端测试和 135 条 Rust 全量测试通过；Prettier、ESLint、TypeScript、Vite build、Rust fmt、Clippy、cargo check、git diff check 与 Tauri release build 均通过；本模块无 UI 变更，不需要视觉验收；模块提交 `00bc98c` 已推送到 `origin/main`。
