...
- **模块 51：应用配置仓库删除** 已完成实现与审查：新增即时事务 `Database::delete_application`，写前验证完整存储，删除目标、压缩后续位置并返回权威有序快照；新增 `ApplicationNotFound` 存储错误和共享 `application_not_found` Rust/TypeScript IPC 错误契约。测试覆盖中部/首部/尾部删除及每步重开、未知 ID 原子失败和损坏存储拒绝删除。首轮 review 发现前端运行时错误白名单缺失新 code（P2），现已补齐联合类型、白名单和规范化回归测试；`gpt-5.6-sol medium` 最终复审结果为 `APPROVE`，无 P1/P2。26 条应用聚焦 Rust 测试、44 条 IPC 客户端聚焦测试、118 条前端测试和 135 条 Rust 全量测试通过；Prettier、ESLint、TypeScript、Vite build、Rust fmt、Clippy、cargo check、git diff check 与 Tauri release build 均通过；本模块无 UI 变更，不需要视觉验收。
