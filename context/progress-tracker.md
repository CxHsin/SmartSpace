...
- **模块 49：应用配置仓库读取** 已完成实现与审查：新增只读 `Database::list_applications`，按 `position ASC, id ASC` 读取五列并恢复完整 `ApplicationConfig`；拒绝无效 UUID、非规范显示名称/图标键、相对/NUL/非 `.exe` 路径、非声明 SQLite 列类型以及重复或间断位置，统一返回 `CorruptApplicationStore`，且损坏读取不写入、不修复、不丢弃数据。首轮 review 发现该错误未映射到共享 `data_corrupt` code（P2），现已补齐映射与序列化回归测试；`gpt-5.6-sol medium` 最终复审结果为 `APPROVE`，无 P1/P2。18 条应用聚焦 Rust 测试、117 条前端测试和 127 条 Rust 全量测试通过；Prettier、ESLint、TypeScript、Vite build、Rust fmt、Clippy、cargo check、git diff check 与 Tauri release build 均通过；本模块无 UI 变更，不需要视觉验收。
