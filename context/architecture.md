...
- 应用配置创建在即时写事务中验证现有存储不变量并追加到当前位置序列末尾；仓库只做路径语法校验，不检查文件是否存在，也不负责图标提取或进程启动。
- 应用配置删除在即时写事务中验证目标与存储不变量，删除后原子压缩后续位置并返回完整有序配置快照；缺失目标使用稳定的 `ApplicationNotFound` 错误，不把持久配置删除与运行进程关闭混为同一仓库职责。
